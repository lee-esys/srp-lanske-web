...
ダイアログ上部に前後移動操作と現在位置を表示する。

```text
[←]  R 3 / C 1  [→]    （試合前｜試合中｜終了）
```

ナビゲーションと状態選択が横幅へ収まらない場合は、固定の画面幅判定でレイアウトを切り替えず、`Wrap`によって自然に次の行へ折り返す。
...
ダイアログを閉じた時点で`ScheduleRoundsView`のprogressを1回再読込し、次をまとめて最新状態へ反映する。

- 対象を含む試合カードの状態・スコア
- ラウンド完了表示
- 進行summary

ブラウザローカルの対戦表履歴は保存時に更新する。
